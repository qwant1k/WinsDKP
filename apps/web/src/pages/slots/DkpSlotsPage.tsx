...
.sl-logo {
  ...
  font-size: clamp(20px, 2.6vw, 32px); letter-spacing: 0.12em;
...
.sl-bal-lbl  { font-family: 'Orbitron', monospace; font-size: 11px; letter-spacing: 0.2em; color: #3d2a5a; text-transform: uppercase; margin-bottom: 1px; }
.sl-bal-val  { font-family: 'Orbitron', monospace; font-size: clamp(26px,3.4vw,38px); font-weight: 700; color: #e879f9; text-shadow: 0 0 18px rgba(232,121,249,0.6); line-height: 1; }
...
.sl-win-line {
  ...
  height: 150px; top: 50%; transform: translateY(-50%);
...
.sl-payout-chip {
  padding: 8px 14px; border-radius: 20px; font-size: 15px;
...
/* ── spin button ── */
.sl-spin-btn {
  width: 100%; padding: 18px 26px; border-radius: 14px; border: none; cursor: pointer;
...
function Reel({ targetSymbol, spinning, delay, onStop }: {
  ...
  useEffect(() => {
    ...
    const minY = -stripH + REEL_H;
    const maxY = 0;
    let forceStopTimer: ReturnType<typeof setTimeout> | undefined;

    // target position: choose a reachable index for the symbol.
    const symbolIdx = strip
      .map((s, idx) => ({ s, idx }))
      .filter(({ s }) => s === targetSymbol)
      .map(({ idx }) => idx);
    const reachableIdx = symbolIdx.filter((idx) => {
      const y = -(idx * CELL_H - CELL_H);
      return y >= minY && y <= maxY;
    });
    const targetIdx = reachableIdx.length > 0
      ? reachableIdx[reachableIdx.length - 1]
      : symbolIdx[symbolIdx.length - 1] ?? 1;
    // center it in the middle visible slot (3 cells per reel).
    ...
    const tick = () => {
      ...
      // wrap
      if (posRef.current < minY) posRef.current += stripH - STRIP * CELL_H;
      setY(posRef.current);
    ...
    forceStopTimer = setTimeout(() => {
      posRef.current = Math.max(minY, Math.min(targetY, maxY));
      setY(posRef.current);
      setBouncing(true);
      setTimeout(() => { setBouncing(false); onStop(); }, 350);
    }, startDelay + 9000);
    ...
    <div className="sl-overlay" onClick={onClose}>
      <div className="sl-modal" onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '24px 22px 20px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          {isJackpot && (
            <div style={{
              fontFamily: 'Orbitron, monospace', fontSize: 12, letterSpacing: '0.3em',
          ...
          {/* symbol */}
          <div style={{
            fontSize: 70, lineHeight: 1, marginBottom: 8,
          ...
          <div style={{
            ...
            fontSize: 48, color: tc.color, lineHeight: 1,
            textShadow: `0 0 30px ${tc.glow}`, marginBottom: 4,
          }}>×{result.multiplier}</div>

          {/* payout */}
          <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 19, color: '#94a3b8', marginBottom: 4 }}>
            Выигрыш: <span style={{ color: '#f8f8ff', fontWeight: 700, fontSize: 24 }}>{result.payout} DKP</span>
          </div>
          <div style={{
            fontSize: 15, color: result.net >= 0 ? '#4ade80' : '#f87171',
          ...
          <button onClick={() => setProof(p => !p)} style={{
            ...
            color: '#2a1a3a', fontSize: 12, fontFamily: 'Rajdhani', cursor: 'pointer',
          ...
          {proof && (
            <div style={{
              ...
              fontFamily: 'Share Tech Mono, monospace', fontSize: 11, color: '#2a1a3a',
          ...
          <button onClick={onClose} style={{
            ...
            color: '#05020a', fontFamily: 'Orbitron, monospace', fontSize: 15,
...
export function DkpSlotsPage() {
  ...
  const mut = useMutation({
    ...
    onSuccess: (res: SpinResult) => {
      setPending(res);
      // 10% near-miss visual effect. Economy/result stays from backend response.
      if (Math.random() < 0.10) {
        const others = SYMBOLS.filter((s) => s !== res.symbol);
        const rand = () => others[Math.floor(Math.random() * others.length)];
        setSyms([rand(), rand(), res.symbol]);
      } else {
      ...
      <div className="sl-root">
        ...
        <header className="sl-header">
          <div>
            ...
            <div style={{ color: '#2a1a3a', fontSize: 16, marginTop: 3, fontFamily: 'Rajdhani' }}>
        ...
        <div className="sl-body">
          ...
          <div className="sl-left">
            ...
            {/* CABINET */}
            <div className="sl-cabinet" ref={cabinetRef} style={{ width: '80%', maxWidth: 980, height: '80%', minHeight: 0 }}>
              ...
              <div className="sl-multiplier" style={{
                ...
                {spinning
                  ? <span style={{ color: '#2a1a3a', fontSize: 20, letterSpacing: '0.2em' }}>ВРАЩЕНИЕ…</span>
                ...
                {(info?.tiers ?? []).map(t => {
                  ...
                  return (
                    <div key={t.id} className="sl-payout-chip" style={{
                      ...
                      {t.payout} DKP
                      <span style={{ opacity: 0.5, marginLeft: 4, fontSize: 12 }}>{t.chance}%</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* SPIN BUTTON */}
            <div style={{ width: '80%', maxWidth: 980 }}>
          ...
          <div className="sl-right">
                  ...
                  return (
                    <div key={t.id} className="sl-pt-row">
                      ...
                      <div style={{
                        ...
                        fontSize: 28, color: tc2.color, textShadow: `0 0 10px ${tc2.glow}`,
                      }}>{tc2.glyph}</div>
                      {/* Label + multiplier */}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{
                          fontFamily: 'Orbitron, monospace', fontSize: 16, color: tc2.color,
                          letterSpacing: '0.08em',
                        }}>{t.label} ×{t.multiplier}</div>
                        <div style={{ fontSize: 17, color: '#2a1a3a', fontFamily: 'Rajdhani' }}>
                          Шанс: {t.chance}%
                        </div>
                      </div>
                      {/* Payout */}
                      <div style={{
                        ...
                        fontSize: 22, color: tc2.color, flexShrink: 0,
            ...
            <div className="sl-panel" style={{ flex: '1 1 0', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
              ...
              <div className="sl-panel-body" style={{ padding: '4px 0' }}>
                {logs.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '28px 16px', color: '#1a0c28', fontStyle: 'italic', fontSize: 17, fontFamily: 'Rajdhani' }}>
                    Нет истории…
                  </div>
                ) : logs.map((log, i) => {
                  ...
                  return (
                    <div key={log.id} className="sl-log-row sl-fadein"
                      ...
                      <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 16, color: tc2.color, fontFamily: 'Orbitron',
                      }}>
                        {log.user.avatarUrl
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap: 4 }}>
                          <span style={{ color:'#cbd5e1', fontFamily:'Rajdhani', fontSize:18, fontWeight:700, overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{log.user.username}</span>
                          <span style={{ color:'#1a0c28', fontSize:13 }}>·</span>
                          <span style={{ color: tc2.color, fontSize:16, fontFamily:'Share Tech Mono', flexShrink:0 }}>×{log.multiplier}</span>
                        </div>
                        <div style={{ fontSize:17, color: isWin ? '#4ade80' : '#f87171', fontFamily:'Share Tech Mono' }}>
                          {isWin ? '+' : ''}{(log.payout - 5).toFixed(1)} DKP
                        </div>
                      </div>
                      <span style={{ color:'#1a0c28', fontSize:16, flexShrink:0, fontFamily:'Share Tech Mono' }}>{ago}</span>
                    </div>
                  );
                })}
              </div>
